...
public class FileSystemMcpServerToolsTests : IDisposable
{
    ...
    [Fact]
    public void ListFilesInDirectory_ShouldReturnFilesInBaseDirectory()
    {
        // Arrange - Create a fresh test file for this test
        string testFilePath = Path.Combine(_testDirectory, "test.txt");
        File.WriteAllText(testFilePath, "Test content");

        string testSubDirectory = Path.Combine(_testDirectory, "subdir");
        Directory.CreateDirectory(testSubDirectory);

        // Act
        var files = FileSystemMcpServerTools.ListFilesInDirectory(_testDirectory);

        // Assert
        Assert.Contains("test.txt", files);
        Assert.Contains("subdir/", files);
        Assert.Equal(2, files.Length); // One file and one directory
    }
